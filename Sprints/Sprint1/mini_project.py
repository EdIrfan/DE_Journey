...
# CONFIGURATION
# ============================================================================
# Define rules for validation. These are the "contracts" your data must follow.

# Expected data types for each column
# Why? Schema changes break downstream code (e.g., int becomes string).
EXPECTED_SCHEMA = {
    'Sprint': 'object',
    'Sprint_Name': 'object',
    'Phase': 'object',
...
# Why? Typos in enums cause silent bugs (e.g., "Foundatin" vs "Foundation").
VALID_VALUES = {
    'Phase': ['Foundation', 'Core Tools', 'Capstone', 'Revision', 'Advanced Concepts', 'Software Engineering', 'Cloud & Infrastructure', 'Interview Prep'],
    'Type': ['Setup', 'Study', 'Practice', 'Design', 'Implementation', 'Documentation', 'Interview Prep', 'Job Search', 'Project', 'Study & Practice', 'Revision']
}

# Business rules for numeric fields
MIN_EST_HOURS = 0.5
MAX_EST_HOURS = 100
...
def validate_schema(df):
    ...
    Why this matters:
    - A missing column crashes downstream code.
    - Type mismatch (string instead of int) causes silent errors in calculations.
    
    What we check:
    1. All required columns present?
    2. Each column has the correct data type?
    """
    logger.info("=" * 70)
    ...
    for col, expected_type in EXPECTED_SCHEMA.items():
        ...
        if expected_type in actual_type:
            logger.info(f"  ✓ '{col}' → {actual_type} (correct)")
        else:
            logger.warning(f"  ✗ '{col}' → Expected {expected_type}, got {actual_type}")
            issues += 1
...
def check_nulls(df):
    ...
    Why this matters:
    - NULLs in ID columns = broken joins, data loss.
    - NULLs in optional fields may be acceptable, but you need to know.
    - High NULL ratio may indicate upstream pipeline failure.
    
    What we check:
    1. Which columns have NULLs?
    2. How many NULLs in each column?
    3. Row indices with NULLs (for debugging).
    """
    logger.info("=" * 70)
...
def check_full_duplicates(df):
    ...
    Why this matters:
    - Full duplicates usually mean human error (copy-paste) or ETL bug (double insert).
    - Having exact duplicates violates data uniqueness principles.
    
    What we check:
    1. How many rows are complete duplicates?
    2. Show the duplicate rows (for root cause analysis).
    """
    logger.info("=" * 70)
    ...
    if n_full_dupes > 0:
        ...
        logger.warning(f"Result: FAILED (duplicates found)\n")
    else:
...
def check_key_duplicates(df):
    ...
    - Duplicate IDs = can't join with other tables reliably.
    - Violates basic database integrity principles.
    
    What we check:
    1. Are all Story_IDs unique?
    2. If not, which IDs are duplicated and how many times?
    """
    logger.info("=" * 70)
    ...
    # keep=False marks all occurrences (duplicates AND first occurrence)
    ...
    if n_key_dupes > 0:
        ...
        logger.error(f"Duplicate Story_IDs and their counts:")
        ...
        logger.error(f"Result: FAILED (primary key violation)\n")
    else:
        logger.info("  ✓ All Story_IDs are unique")
        logger.info("Result: PASSED ✓\n")
...
def validate_business_logic(df):
    ...
    Why this matters:
    - A number can be technically valid (integer, not NULL) but logically wrong.
    - Example: Est_Hours = 999 is valid int but obviously wrong for a task.
    - Business logic catches human errors that type checks miss.
    
    What we check:
    1. Est_Hours in realistic range? (0.5 to 100 hours)
    2. Phase values are from approved list? (catch typos like "Foundatin")
    3. Type values are from approved list? (enforce enum integrity)
    """
    logger.info("=" * 70)
    ...
    if 'Est_Hours' in df.columns:
        ...
        if not invalid_hours.empty:
            ...
            logger.info("  ✓ All Est_Hours values are in valid range")
    ...
    if 'Phase' in df.columns:
        ...
        if not invalid_phases.empty:
            ...
            logger.info("  ✓ All Phase values are valid")
    ...
    if not issues_found:
        ...
        logger.warning(f"Result: FAILED (business logic violations found)\n")
...
def calculate_data_quality_score(df):
    ...
    Why this matters (ADVANCED CONCEPT):
    - Management needs a single metric to understand data health.
    - Helps track improvement over time (trending).
    - Defines SLAs (Service Level Agreements): "Data quality must be > 80%".
    
    How it works:
    - Start with 100 points.
    - Lose points for each issue category:
      * NULLs: up to -20 points
      * Duplicates: up to -15 points
      * Invalid enums: up to -15 points
      * Invalid numeric values: up to -10 points
    - Final score tells if data is "good" (>90), "fair" (70-90), or "bad" (<50).
    
    This is what you'll use in real jobs to report data quality to leadership.
    """
    logger.info("=" * 70)
    ...
    null_deduction = min(20, null_ratio / 5)  # Max -20 points
    ...
    dupe_deduction = min(15, dupe_ratio / 6.67)  # Max -15 points
    ...
    if 'Phase' in df.columns:
        ...
        phase_deduction = min(15, invalid_phase_ratio / 6.67)  # Max -15 points
    ...
    if 'Est_Hours' in df.columns:
        ...
        hours_deduction = min(10, invalid_hours_ratio / 10)  # Max -10 points
...
def main():
    ...
    Each step is logged so you can trace exactly where issues occur.
    """
    ...
    try:
        ...
        logger.info("=" * 70)
        logger.info(f"Full report saved to: validation_report.log\n")
